...
/**
 * Created by dtw on 15/6/2.
 */
public class WifiStateReceiver extends BroadcastReceiver{
    @Override
    public void onReceive(Context context, Intent intent) {
        if (MyApplication.autochangedevicewifi) {
                ...
                if (null != parcelableExtra) {
                    ...
                    if (isConnected) {
                        ...
                        if (wifiAdmin.getWifiSSID().contains("IMT_DEVICE")) {
                            EventBus.getDefault().post(new APConnectedEvent());
                            Log.v("wifistate", "connect");
                        } else {
                            EventBus.getDefault().post(new WifiConnectEvent());
                            Log.v("wifistate", "netconnect");
                        }
                    } else {
                        if (MyApplication.wifiinfosended) {
                            EventBus.getDefault().post(new APDisconnectEvent());
                            MyApplication.wifiinfosended = false;
                            Log.v("wifistate", "wifidisconnect");
                        }
                        else if (MyApplication.iswanttoconnectwifi) {
